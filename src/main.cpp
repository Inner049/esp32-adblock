...
static bool loadWifiCfg() {
  File f = LittleFS.open("/wifi.cfg", "r"); if (!f) return false;
  cfgSSID = ""; cfgPass = "";
  while (f.available()) {
  ...
  f.close(); return cfgSSID.length() > 0;
}
static void saveWifiCfg(const String& ssid, const String& pass) {
  File f = LittleFS.open("/wifi.cfg", "w"); if (!f) return;
  f.println("ssid=" + ssid); f.println("pass=" + pass); f.close();
}
static void loadDnsCfg() {
  File f = LittleFS.open("/dns.cfg", "r"); if (!f) return;
  String line = f.readStringUntil('\n'); line.trim();
  if (line.startsWith("upstream=")) { String ip = line.substring(9); ip.trim(); upstreamDNS.fromString(ip); }
  f.close();
}
static void saveDnsCfg() {
  File f = LittleFS.open("/dns.cfg", "w"); if (!f) return;
  f.println("upstream=" + upstreamDNS.toString()); f.close();
}
static void loadLangCfg() {
  File f = LittleFS.open("/lang.cfg", "r"); if (!f) return;
  String line = f.readStringUntil('\n'); line.trim();
  if (line == "uk" || line == "ru" || line == "en") currentLang = line;
  f.close();
}
static void saveLangCfg() { File f = LittleFS.open("/lang.cfg", "w"); if (f) { f.println(currentLang); f.close(); } }
...
static void loadCloudCfg() {
  File f = LittleFS.open("/cloud.cfg", "r"); if (!f) return;
  firebaseDbUrl = f.readStringUntil('\n'); firebaseDbUrl.trim();
  f.close();
}
static void saveCloudCfg() {
  File f = LittleFS.open("/cloud.cfg", "w"); if (!f) return;
  f.println(firebaseDbUrl); f.close();
}
...
static Dev* getClient(uint32_t ip) {
  ...
  if (numClients < MAX_CLIENTS) {
    Dev* c = &clients[numClients++]; c->ip = ip; c->blocked = c->allowed = 0; c->lastSeen = millis(); c->banned = isBannedIP(ip); c->label = "";
...
static int forwardUpstream(int qlen) {
  upstreamCli.beginPacket(upstreamDNS, 53); upstreamCli.write(buf, qlen); upstreamCli.endPacket();
  uint32_t t0 = millis();
...
static void handleApDns() {
  ...
  int qlen = dnsServer.read(buf, sizeof(buf)); if (qlen < 13) return;
  char domain[256]; uint16_t qtype = 0; int qend = qlen;
  if (parseQuery(buf, qlen, domain, &qtype, &qend) == 0) return;
  buf[2] = 0x81; buf[3] = 0x80; buf[6] = 0; buf[7] = (qtype == 1) ? 1 : 0; buf[8] = 0; buf[9] = 0; buf[10] = 0; buf[11] = 0;
  int rlen = qend;
  if (qtype == 1) { const uint8_t ans[] = {0xC0,0x0C, 0,1, 0,1, 0,0,0,60, 0,4, 192,168,4,1}; memcpy(buf + qend, ans, sizeof(ans)); rlen = qend + sizeof(ans); }
  dnsServer.beginPacket(cip, cport); dnsServer.write(buf, rlen); dnsServer.endPacket();
}
...
const char SETUP_PAGE[] PROGMEM = R"HTML(<!doctype html><html><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1">
...
const char DASH_PAGE[] PROGMEM = R"HTML(<!doctype html><html><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1">
...
static void handleSetLang() {
  String l = web.arg("l"); l.trim(); l.toLowerCase();
  if (l == "uk" || l == "ru" || l == "en") { currentLang = l; saveLangCfg(); }
  web.send(200, "text/plain", "ok");
}
static void handleScanJson() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    j += "{\"ssid\":\"" + jesc(ssid) + "\",\"rssi\":" + WiFi.RSSI(i) + ",\"enc\":" + String(WiFi.encryptionType(i) != WIFI_AUTH_OPEN ? "true" : "false") + "}";
  }
  j += "]}"; WiFi.scanDelete(); web.send(200, "application/json", j);
}
static void handleSaveWifi() {
  String ssid = web.arg("s"); ssid.trim(); String pass = web.arg("p");
  if (ssid.length() == 0) { web.send(400, "text/plain", "empty ssid"); return; }
  saveWifiCfg(ssid, pass); web.send(200, "text/plain", "ok"); delay(1000); ESP.restart();
}
static void handleSetDns() {
  IPAddress dns; if (dns.fromString(web.arg("ip"))) { upstreamDNS = dns; saveDnsCfg(); }
  web.send(200, "text/plain", "ok");
}
static void handleFactoryReset() {
  LittleFS.remove("/wifi.cfg"); LittleFS.remove("/dns.cfg"); LittleFS.remove("/lang.cfg");
  web.send(200, "text/plain", "ok"); delay(500); ESP.restart();
}

// -- DNS benchmark --
static uint8_t benchQ[29] = {
  0x00,0x00, 0x01,0x00, 0x00,0x01, 0x00,0x00, 0x00,0x00, 0x00,0x00,
  7,'e','x','a','m','p','l','e', 3,'c','o','m', 0, 0x00,0x01, 0x00,0x01
};
static void doBenchmark(IPAddress dns, int count, uint32_t& outMin, uint32_t& outAvg, uint32_t& outMax) {
  WiFiUDP bUdp; bUdp.begin(0); uint32_t minT = 99999, maxT = 0, sumT = 0;
  for (int i = 0; i < count; i++) {
    benchQ[0] = random(256); benchQ[1] = random(256);
    uint32_t t0 = micros();
    bUdp.beginPacket(dns, 53); bUdp.write(benchQ, 29); bUdp.endPacket();
    bool got = false; uint32_t deadline = millis() + 3000;
    while (millis() < deadline) {
      if (bUdp.parsePacket() > 0) {
        uint8_t tmp[64]; bUdp.read(tmp, sizeof(tmp));
        uint32_t dt = (micros() - t0) / 1000;
        if (dt < minT) minT = dt; if (dt > maxT) maxT = dt;
        sumT += dt; got = true; break;
      } delay(1);
    }
    if (!got) { sumT += 3000; if (3000 > maxT) maxT = 3000; }
  }
  bUdp.stop(); outMin = (minT == 99999) ? 0 : minT; outAvg = count > 0 ? sumT / count : 0; outMax = maxT;
}
static void handleBenchmark() {
  IPAddress dns; if (!dns.fromString(web.arg("ip"))) { web.send(400, "text/plain", "bad ip"); return; }
  uint32_t mn, avg, mx; doBenchmark(dns, 15, mn, avg, mx);
  web.send(200, "application/json", "{\"min\":" + String(mn) + ",\"avg\":" + String(avg) + ",\"max\":" + String(mx) + "}");
}
static void handleBenchmarkAll() {
  const char* names[] = {"Quad9","Cloudflare","Google","OpenDNS"};
  IPAddress ips[4] = {IPAddress(9,9,9,9), IPAddress(1,1,1,1), IPAddress(8,8,8,8), IPAddress(208,67,222,222)};
  String j = "{";
  for (int s = 0; s < 4; s++) {
    uint32_t mn, avg, mx; doBenchmark(ips[s], 5, mn, avg, mx);
    if (s) j += ","; j += "\"" + String(names[s]) + "\":{\"min\":" + String(mn) + ",\"avg\":" + String(avg) + ",\"max\":" + String(mx) + "}";
  }
...
static void beginBlocklistSwap() {
  if (blocklist) blocklist.close(); numHashes = 0;
  LittleFS.remove(BLOCKLIST_PATH); LittleFS.remove("/blocklist.new");
}
static bool commitNewBlocklist() {
  ...
  if (ok) LittleFS.rename("/blocklist.new", BLOCKLIST_PATH); else LittleFS.remove("/blocklist.new");
  reopenBlocklist(); return ok;
}

static bool upOk = false; static File upFile;
static void handleUploadDone() { web.send(upOk ? 200 : 500, "text/plain", upOk ? "ok" : "rejected"); }
static void handleUpload() {
  HTTPUpload& u = web.upload();
  if (u.status == UPLOAD_FILE_START) { upOk = false; beginBlocklistSwap(); upFile = LittleFS.open("/blocklist.new", "w"); }
  else if (u.status == UPLOAD_FILE_WRITE) { if (upFile) upFile.write(u.buf, u.currentSize); }
  else if (u.status == UPLOAD_FILE_END) { if (upFile) upFile.close(); upOk = commitNewBlocklist(); }
  else if (u.status == UPLOAD_FILE_ABORTED) { if (upFile) upFile.close(); LittleFS.remove("/blocklist.new"); reopenBlocklist(); }
}

static void checkFirmwareUpdate() {
  if (!fwUpdateUrl.length()) return;
  Serial.println("[OTA] Checking for firmware update...");
  WiFiClientSecure client; client.setInsecure(); HTTPClient http;
  
  String verUrl = fwUpdateUrl;
  if (!verUrl.endsWith("/")) verUrl += "/";
  if (http.begin(client, verUrl + "version.txt")) {
    ...
    if (code == HTTP_CODE_OK) {
      ...
      if (remoteVer > FW_VERSION) {
        ...
        t_httpUpdate_return ret = httpUpdate.update(client, verUrl + "firmware.bin");
...
static bool fetchBlocklist(String url) {
  ...
  WiFiClientSecure cs; cs.setInsecure(); WiFiClient cl; HTTPClient http; http.setTimeout(20000); http.setFollowRedirects(HTTPC_STRICT_FOLLOW_REDIRECTS);
  if (!(url.startsWith("https") ? http.begin(cs, url) : http.begin(cl, url))) { updateStatus = "begin failed"; return false; }
  int code = http.GET(); if (code != HTTP_CODE_OK) { http.end(); updateStatus = "HTTP " + String(code); return false; }
  beginBlocklistSwap(); File f = LittleFS.open("/blocklist.new", "w"); if (!f) { http.end(); updateStatus = "fs open failed"; reopenBlocklist(); return false; }
  WiFiClient* stream = http.getStreamPtr(); int len = http.getSize(); uint8_t b[1024]; size_t total = 0; uint32_t idle = millis();
  ...
  f.close(); http.end(); bool ok = commitNewBlocklist();
  updateStatus = ok ? ("ok: " + String(numHashes) + " domains") : ("bad data (" + String(total) + "B)");
  return ok;
}

static void handleFwUpdateDone() {
  bool ok = !Update.hasError(); web.send(ok ? 200 : 500, "text/plain", ok ? "ok" : "failed"); if (ok) { delay(300); ESP.restart(); }
}
...
static void startMainServices() {
  ...
  web.on("/setcloud", []() {
    if (web.hasArg("u")) firebaseDbUrl = web.arg("u");
    saveCloudCfg(); web.send(200, "text/plain", "ok");
  });
...
void setup() {
  Serial.begin(115200);
  pinMode(LED_PIN, OUTPUT); digitalWrite(LED_PIN, HIGH); delay(300);
  Serial.println("\n[c3-adblock] booting");
  ...
  loadCustom(); loadBanned(); loadUpdateCfg(); loadCloudCfg(); loadDnsCfg(); loadLangCfg();
  
  checkBootButton();

  if (loadWifiCfg()) {
    deviceMode = MODE_MAIN;
    WiFi.mode(WIFI_STA); WiFi.setSleep(false); WiFi.setAutoReconnect(true); WiFi.begin(cfgSSID.c_str(), cfgPass.c_str());
    uint32_t t0 = millis();
    while (WiFi.status() != WL_CONNECTED && millis() - t0 < 10000) {
      delay(300); Serial.print("."); digitalWrite(LED_PIN, !digitalRead(LED_PIN));
    }
    if (WiFi.status() == WL_CONNECTED) startMainServices();
  } else {
    deviceMode = MODE_SETUP; startSetupMode();
  }
}
...
void loop() {
  ...
  if (deviceMode == MODE_SETUP) {
    handleApDns(); web.handleClient();
    static uint32_t lb = 0; if (millis() - lb > 1000) { lb = millis(); digitalWrite(LED_PIN, !digitalRead(LED_PIN)); }
    delay(1); return;
  }
  ...
  if (!servicesStarted) {
    if (WiFi.status() == WL_CONNECTED) {
      ...
      static uint32_t lb2 = 0; if (millis() - lb2 > 300) { lb2 = millis(); digitalWrite(LED_PIN, !digitalRead(LED_PIN)); }
      static uint32_t lr = 0; if (lr == 0) lr = millis();
      if (millis() - lr > 10000) { lr = millis(); WiFi.disconnect(); WiFi.begin(cfgSSID.c_str(), cfgPass.c_str()); }
      delay(100); return;
    }
  }

  ArduinoOTA.handle(); web.handleClient(); handleDns();

  if (WiFi.status() != WL_CONNECTED) {
    static uint32_t lb3 = 0; if (millis() - lb3 > 200) { lb3 = millis(); digitalWrite(LED_PIN, !digitalRead(LED_PIN)); }
  } else {
    digitalWrite(LED_PIN, LOW);
  }
  ...
  if (firebaseDbUrl.length()) {
    uint32_t now = millis();
    if (lastTelemetryMs == 0 || now - lastTelemetryMs >= 300000UL) { // every 5 minutes
      lastTelemetryMs = now;
      ...
      String payload = "{\"ip\":\"" + WiFi.localIP().toString() + "\",\"uptime\":" + String(millis() / 1000) + 
                       ...
                       ",\"heap\":" + String(ESP.getFreeHeap()) + ",\"lastSeen\": { \".sv\": \"timestamp\" }}";
                       
      WiFiClientSecure client; client.setInsecure();
      HTTPClient http;
      if (firebaseDbUrl.startsWith("https")) http.begin(client, url);
      else http.begin(url);
      ...
      if (code == HTTP_CODE_OK) {
        ...
        if (cmd == "reboot") {
          http.begin(client, cmdUrl); http.addHeader("Content-Type", "application/json"); http.PUT("null"); http.end(); // clear command
          ESP.restart();
  ...
  if (fwUpdateUrl.length()) {
    uint32_t now = millis();
    if (lastFwCheckMs == 0 || now - lastFwCheckMs >= 86400000UL) { // daily check
      lastFwCheckMs = now;
      checkFirmwareUpdate();
    }
  }
